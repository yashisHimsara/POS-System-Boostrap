...
$('#orderFormCstId').on('change', function(){
    ...
    for (let customerArElement of customer) {
        if (customerArElement.id==selectedCustomerId){
            $('#orderFormCustomerName').val(customerArElement.name);
            $('#orderFormCustomerSalary').val(customerArElement.salary);
            $('#orderFormCustomerAddress').val(customerArElement.address);
        }
    }
});
